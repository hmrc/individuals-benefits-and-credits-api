...
class WorkingTaxCreditController @Inject()(
  ...
  def workingTaxCredit(matchId: UUID, startDate: LocalDate, endDate: LocalDate): Action[AnyContent] =
    Action.async { implicit request =>
      ...
      authenticate(scopes, matchId.toString) { authScopes =>
        ...
        taxCreditsService
          .getWorkingTaxCredits(matchId, startDate, endDate, authScopes)
          .map(
            applications => {
              val selfLink = HalLink(
                "self",
                urlWithInterval(s"/individuals/benefits-and-credits/working-tax-credits?matchId=$matchId", startDate))
              val response =
